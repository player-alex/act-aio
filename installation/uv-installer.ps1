...
function Download($download_url, $platforms) {
  ...
  $local_binary_path = Join-Path $script_dir "binaries\$artifact_name"

  if (-not (Test-Path $local_binary_path)) {
    throw "ERROR: Local binary not found at $local_binary_path. Please ensure the binary is placed in the binaries directory."
  }

  Write-Information "Installing $app_name $app_version ($arch) from local binary"
  Write-Verbose "  from $local_binary_path"
  Write-Verbose "  to $dir_path"

  Copy-Item $local_binary_path -Destination $dir_path
